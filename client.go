...
// ChzzkClient는 치지직 Open API 호출을 위한 클라이언트이다.
//
// [NewChzzkClient]로 생성한다. OAuth 토큰이 필요한 API를 호출하려면
// [ChzzkClient.SetTokens]로 토큰을 주입하거나 [LoginServer]를 통해 로그인해야 한다.
// 토큰의 저장과 복원은 사용자 책임이며, 갱신된 토큰은 [ChzzkClient.OnTokenRefresh]
// 콜백으로 전달받아 저장할 수 있다.
...
// NewChzzkClient는 새 [ChzzkClient]를 생성한다.
//
// clientID와 clientSecret은 치지직 개발자 센터에서 발급받은 값을,
// redirectURI는 애플리케이션에 등록한 리디렉션 URI를 전달한다.
func NewChzzkClient(clientID, clientSecret, redirectURI string) *ChzzkClient {
	c := &ChzzkClient{
		ClientID:     clientID,
...
func (c *ChzzkClient) SetBaseURL(u string) {
	c.baseURL = u
}

// SetLogger는 클라이언트 내부 동작(토큰 갱신 등)을 기록할 로거를 설정한다.
// 설정하지 않으면 아무것도 기록하지 않는다.
func (c *ChzzkClient) SetLogger(l *slog.Logger) {
	c.auth.mu.Lock()
